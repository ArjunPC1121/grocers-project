import { useNavigate } from "react-router-dom";
import type { Product } from "../types";
import { Heart, Plus, Star } from "lucide-react";
import { useAuthenticatedCart } from "../hooks/useAuthenticatedCart";
import { useWishlist } from "../hooks/useWishlist";

interface Props {
  product: Product;
}

const ProductCard = ({ product }: Props) => {
  const currency = "₹";
  const outOfStock = product.stock <= 0;

  const addToAuthenticatedCart = useAuthenticatedCart();
  const addToWishlist = useWishlist();
  const navigate = useNavigate();

  return (
      <div
          className={`bg-white rounded-2xl overflow-hidden shadow transition-all duration-300 group animate-fade-in cursor-pointer ${
              outOfStock
                  ? "opacity-60 grayscale"
                  : "hover:shadow-md"
          }`}
          onClick={() => navigate(`/products/${product.id}`)}
      >
        <div className="relative aspect-square overflow-hidden">
          <img
              src={product.image}
              alt={product.name}
              className="w-full h-full object-cover p-4 group-hover:p-2 transition-all duration-300"
          />

          <button
              onClick={(event) => {
                event.stopPropagation();
                void addToWishlist(product.id);
              }}
              className="absolute top-3 right-3 z-10 size-8 rounded-full bg-white/90 text-app-orange flex-center hover:bg-white"
              title="Add to wishlist"
          >
            <Heart className="size-4" />
          </button>

          <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
            {product.discount > 0 && (
                <span className="px-2 py-0.5 text-[10px] font-semibold uppercase bg-app-orange text-white rounded-full">
              {product.discount}% OFF
            </span>
            )}

            {outOfStock && (
                <span className="px-2 py-0.5 text-[10px] font-semibold uppercase bg-red-600 text-white rounded-full shadow-sm">
              Out of Stock
            </span>
            )}
          </div>
        </div>

        <div className="p-3.5 text-zinc-700">
          <h3 className="text-sm leading-snug mb-1.5 line-clamp-2">
            {product.name}
          </h3>

          {product.rating > 0 && (
              <div className="flex items-center gap-1 mb-2">
                <Star className="size-3 text-app-warning fill-app-warning" />

                <span className="text-xs font-medium text-app-text">
              {product.rating}
            </span>

                <span className="text-xs text-app-text-light">
              ({product.reviewCount})
            </span>
              </div>
          )}

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1 truncate">
            <span className="text-base font-medium">
              {currency}
              {product.price.toFixed(1)}
            </span>

              <span className="text-xs text-app-text-light block">
              /{product.unit}
            </span>

              {product.originalPrice > product.price && (
                  <span className="text-xs text-app-text-light line-through ml-1.5">
                {currency}
                    {product.originalPrice.toFixed(1)}
              </span>
              )}
            </div>

            <button
                disabled={outOfStock}
                onClick={(event) => {
                  event.stopPropagation();

                  if (!outOfStock) {
                    void addToAuthenticatedCart(product);
                  }
                }}
                title={outOfStock ? "Out of stock" : "Add to cart"}
                className="size-7 rounded-full bg-app-orange text-white flex-center shrink-0 hover:bg-app-orange-dark transition-colors active:scale-95 disabled:cursor-not-allowed disabled:bg-zinc-400 disabled:hover:bg-zinc-400 disabled:opacity-100"
            >
              <Plus className="size-3.5" />
            </button>
          </div>
        </div>
      </div>
  );
};

export default ProductCard;
