  ...
  return <main className="min-h-screen bg-app-cream p-6 flex-center">
    <form onSubmit={async (event) => {
      ...
      {customerRegistration && (
        <label className="block mt-4 text-sm font-medium">
          ...
          >
            ...
            <option value="FIRST_PET_NAME">What was your first pet&apos;s name?</option>
            <option value="FIRST_SCHOOL_NAME">What was your first school&apos;s name?</option>
      ...
      {customerRegistration && (
        <label className="block mt-4 text-sm font-medium">
          Security answer
          <input required type="password" value={form.secretAnswer || ""}
            onChange={(event) => update("secretAnswer", event.target.value)}
            className="mt-1 w-full rounded-xl border border-app-border p-3" />
        </label>
      )}
      <button className="mt-6 w-full rounded-xl bg-app-green p-3 text-white font-semibold">{customerRegistration ? "Create account" : "Sign in"}</button>
