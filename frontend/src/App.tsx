...
import { EmployeeDashboard, EmployeeLayout, EmployeeOrders, EmployeeProfile, TicketHistory, UnlockTickets } from "./pages/employee/EmployeeWorkspace";
...
import { AdminRequestQueue, EmployeeProductRequests } from "./pages/ProductRequests";
export default function App() { return <><Toaster position="top-right"/><Routes>
  ...
  <Route element={<RoleGuard allow={["EMPLOYEE"]}/> }><Route path="/employee" element={<EmployeeLayout/>}><Route index element={<EmployeeDashboard/>}/><Route path="requests" element={<EmployeeProductRequests/>}/><Route path="orders" element={<EmployeeOrders/>}/><Route path="tickets" element={<UnlockTickets/>}/><Route path="ticket-history" element={<TicketHistory/>}/><Route path="profile" element={<EmployeeProfile/>}/></Route><Route path="/employee/change-password" element={<EmployeePasswordChange/>}/></Route>
  <Route element={<RoleGuard allow={["ADMIN"]}/> }><Route path="/admin" element={<AdminShell/>}><Route index element={<AdminOverview/>}/><Route path="requests" element={<AdminRequestQueue/>}/>{["orders","products","categories","inventory","customers","employees","delivery","offers","analytics","settings"].map((path)=><Route key={path} path={path} element={<AdminPage/>}/>)}</Route></Route>
