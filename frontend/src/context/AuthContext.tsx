...
type AuthContextType = { user: SessionUser | null; loading: boolean; login: (role: Role, identifier: string, password: string, returnTo?: string) => Promise<void>; register: (payload: Record<string, string>) => Promise<boolean>; logout: () => void; updateUser: (value: Partial<SessionUser>) => void; };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (role: Role, identifier: string, password: string, returnTo?: string) => {
    try {
      ...
      if (next.role === "EMPLOYEE") {
        const profile = await api.get(`/employees/${next.id}`).then(({ data }) => data).catch(() => null);
        if (profile) {
          next = { ...next, firstName: profile.firstName ?? next.firstName, lastName: profile.lastName ?? next.lastName, name: [profile.firstName, profile.lastName].filter(Boolean).join(" ") || next.name, email: profile.email ?? next.email };
          save(next);
        }
      }
      toast.success("Signed in successfully");
      navigate(returnTo || destination(next));
