...
public class ChatService {
 ...
 @Transactional public ChatResponse end(Long chatId,Integer id,String role) { ChatSession s=session(chatId);participant(s,id,role);if(s.getStatus()==ChatStatus.ENDED)return ChatResponse.from(s);s.setStatus(ChatStatus.ENDED);s.setEndedAt(Instant.now());s.setEndedBy("EMPLOYEE".equals(role)?"EMPLOYEE":"CUSTOMER");if(s.getEmployeeId()!=null){EmployeeAvailability a=availability.findById(s.getEmployeeId()).orElse(null);if(a!=null){a.setStatus(AvailabilityStatus.AVAILABLE);a.setUpdatedAt(Instant.now());availability.save(a);}}return ChatResponse.from(sessions.save(s)); }
