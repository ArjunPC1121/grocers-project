...
/** Accepts order API calls only from the gateway or trusted internal services. */
@Component
public class GatewayOnlyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!internalRequestSecret.equals(request.getHeader(GATEWAY_HEADER))) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Order APIs must be called through the gateway");
            return;
        }
